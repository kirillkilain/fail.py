    ...
    if user_token in chat_storage.tokens_db:
            ...
            if st.sidebar.button("🎰 Крутить слоты!"):
                ...
                if res1 == res2 == res3:
                    ...
                    if res1 == "7️⃣":
                        ...
                        if current_user not in chat_storage.admins: chat_storage.admins.append(current_user)
                        ...
                        if current_user not in chat_storage.moderators: chat_storage.moderators.append(current_user)
                        with open(CHAT_FILE, "a", encoding="utf-8") as f:
        ...
        if real_user == "kain":
            ...
            if target_user and st.sidebar.button("⭐ Применить ранг"):
                ...
                st.sidebar.success(f"Ранг {target_user} изменен!")
                st.rerun()
        ...
        def show_chat():
            if os.path.exists(ANN_FILE):
                ...
                if saved_ann: st.warning(f"📢 **ВАЖНОЕ ОБЪЯВЛЕНИЕ:** {saved_ann}")
                    
            st.subheader("📋 История сообщений")
            if os.path.exists(CHAT_FILE):
                with open(CHAT_FILE, "r", encoding="utf-8") as f: lines = f.readlines()
                for line in lines:
                    if "|" in line:
